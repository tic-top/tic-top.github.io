---
layout: v2
title: Resume
permalink: /cv/
---

<section class="section">
  <div class="container">
    <h1 class="section-title">Resume</h1>
    <div style="text-align:center; margin-bottom: 1.5rem;">
      <a class="download-btn" href="{{ '/files/Resume_2024.pdf' | relative_url }}" download>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        Download PDF
      </a>
    </div>

    <h2>Education</h2>
    <div class="timeline">
      <div class="timeline-item">
        <h3>M.S. in Computer Science · University of Michigan</h3>
        <p class="meta">2025 - Present</p>
      </div>
      <div class="timeline-item">
        <h3>B.S. in Computer Science · University of Michigan</h3>
        <p class="meta">2025</p>
      </div>
      <div class="timeline-item">
        <h3>B.S. in ECE · Shanghai Jiao Tong University</h3>
        <p class="meta">2025</p>
      </div>
    </div>

    <h2>Research Experience</h2>
    <div class="timeline">
      <div class="timeline-item">
        <h3>Research Assistant · University of Michigan</h3>
        <p class="meta">Fall 2023 · Supervisor: Lin Ma</p>
        <p>Built a model to predict SQL execution time under different lakehouse settings.</p>
      </div>
      <div class="timeline-item">
        <h3>Research Assistant · University of Michigan</h3>
        <p class="meta">Summer 2023 · Supervisor: Rada Mihalcea</p>
        <p>Evaluated tokenizer performance on PsyQA and benchmarked multiple LLMs.</p>
      </div>
      <div class="timeline-item">
        <h3>Research Assistant · Shanghai Jiao Tong University</h3>
        <p class="meta">Summer 2021 · Supervisor: Fan Wu</p>
        <p>Trained an anomaly sound detection model and deployed it to RZ/T2 board.</p>
      </div>
    </div>

    <h2>Skills</h2>
    <p class="tag-row">
      <span>Python</span><span>Rust</span><span>C/C++</span><span>SQL</span><span>PyTorch</span><span>LaTeX</span>
    </p>
  </div>
</section>
